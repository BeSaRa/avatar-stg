...
export class VideoAnalyzerComponent extends OnDestroyMixin(class {}) implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.listenToVideoInsightsChange()

    setTimeout(() => this.pingActive.set(false), 5000)
  }
  ...
  listenToVideoInsightsChange() {
    this.videoIndexerChatService.videoInsights$
      .pipe(
        takeUntil(this.destroy$),
        tap(video => {
          this.videoAnalyzerService.videoDuration.set(video.res_data.duration)
          this.videoId.set(video.selectedItem.id)
          this.insights.set(video.res_data)

          // Clear chat history and update video-related data
          this.chatContainer()?.clearChatHistory()
          this.video.set(undefined)
          this.videoData.set(video.selectedItem)
        }),
        switchMap(video =>
          this.videoAnalyzerService.getVttFile(video.video_caption).pipe(
            tap(vtt => {
              this.videoCaption.set(vtt)
              this.videoUrl.set(video.video_stream_url)
              this.audioUrl.set(video.audio_stream_url)
              this.videoSummary.set(video.summary)
